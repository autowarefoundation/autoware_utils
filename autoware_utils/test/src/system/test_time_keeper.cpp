...
protected:
  ...
  rclcpp::Publisher<autoware_utils::ProcessingTimeDetail>::SharedPtr publisher;
  std::unique_ptr<autoware_utils::TimeKeeper> time_keeper;

  void SetUp() override
  {
    node = std::make_shared<rclcpp::Node>("test_node");
    publisher = node->create_publisher<autoware_utils::ProcessingTimeDetail>(
      "~/debug/processing_time_tree", 1);
    time_keeper = std::make_unique<autoware_utils::TimeKeeper>(&oss, publisher);
  }
};
