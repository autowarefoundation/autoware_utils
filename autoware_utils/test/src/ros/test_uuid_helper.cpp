...
TEST(UUIDHelperTest, generate_uuid)
{
  ...
  unique_identifier_msgs::msg::UUID uuid1 = autoware::utils::generate_uuid();
  unique_identifier_msgs::msg::UUID uuid2 = autoware::utils::generate_uuid();

  EXPECT_FALSE(uuid1 == uuid2) << "Duplicate UUID generated: "
                               << autoware::utils::to_hex_string(uuid2);
}
...
TEST(UUIDHelperTest, generate_default_uuid)
{
  ...
  unique_identifier_msgs::msg::UUID default_uuid = autoware::utils::generate_default_uuid();
...
TEST(UUIDHelperTest, to_hex_string)
{
  ...
  std::string hex_string = autoware::utils::to_hex_string(uuid);
...
TEST(UUIDHelperTest, to_boost_uuid)
{
  ...
  EXPECT_TRUE(boost_uuid == autoware::utils::to_boost_uuid(uuid));
}
...
TEST(UUIDHelperTest, to_uuid_msg)
{
  ...
  unique_identifier_msgs::msg::UUID ros_uuid = autoware::utils::to_uuid_msg(boost_uuid);
