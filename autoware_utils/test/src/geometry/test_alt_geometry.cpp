...
{
  using autoware_utils::area;
  using autoware_utils::alt::ConvexPolygon2d;
  using autoware_utils::alt::Point2d;
...
{
  using autoware_utils::convex_hull;
  using autoware_utils::alt::PointList2d;
  using autoware_utils::alt::Points2d;
...
{
  using autoware_utils::covered_by;
  using autoware_utils::alt::ConvexPolygon2d;
  using autoware_utils::alt::Point2d;
...
{
  using autoware_utils::envelope;
  using autoware_utils::alt::PointList2d;
  using autoware_utils::alt::Polygon2d;
...
{
  using autoware_utils::intersects;
  using autoware_utils::alt::ConvexPolygon2d;
  using autoware_utils::alt::Point2d;
...
{
  using autoware_utils::is_clockwise;
  using autoware_utils::alt::PointList2d;
...
TEST(alt_geometry, convexHullRand)
{
  ...
  for (auto vertices = 3UL; vertices < max_vertices; ++vertices) {
    ...
    for (auto i = 0UL; i < polygons.size(); ++i) {
      autoware_utils::MultiPoint2d outer;
      for (const auto & point : polygons[i].outer()) {
        outer.push_back(point);
      }
      autoware_utils::Polygon2d ground_truth;
      sw.tic();
      boost::geometry::convex_hull(outer, ground_truth);
      ground_truth_hull_ns += sw.toc();

      const auto vertices =
        autoware_utils::alt::ConvexPolygon2d::create(polygons[i]).value().vertices();
      sw.tic();
      const auto alt = autoware_utils::convex_hull({vertices.begin(), vertices.end()});
      alt_hull_ns += sw.toc();
