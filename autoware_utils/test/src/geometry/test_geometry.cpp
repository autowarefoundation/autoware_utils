...
{
  using autoware_utils::get_longitudinal_velocity;
...
TEST(geometry, set_orientation)
{
  ...
  using autoware_utils::set_orientation;
...
{
  using autoware_utils::set_longitudinal_velocity;
...
{
  using autoware_utils::calc_elevation_angle;
  using autoware_utils::create_point;
  using autoware_utils::deg2rad;
...
TEST(geometry, point_2_tf_vector)
{
  using autoware_utils::create_quaternion_from_rpy;
  using autoware_utils::deg2rad;
  using autoware_utils::point_2_tf_vector;
...
TEST(geometry, inverse_transform_pose)
{
  using autoware_utils::create_quaternion_from_rpy;
  using autoware_utils::deg2rad;
  using autoware_utils::inverse_transform_pose;
...
TEST(geometry, calc_curvature)
{
  ...
  // Straight Line
  {
    geometry_msgs::msg::Point p1 = autoware_utils::create_point(0.0, 0.0, 0.0);
    geometry_msgs::msg::Point p2 = autoware_utils::create_point(1.0, 0.0, 0.0);
    geometry_msgs::msg::Point p3 = autoware_utils::create_point(2.0, 0.0, 0.0);
  ...
  {
    geometry_msgs::msg::Point p1 = autoware_utils::create_point(0.0, 0.0, 0.0);
    geometry_msgs::msg::Point p2 = autoware_utils::create_point(1.0, 1.0, 0.0);
    geometry_msgs::msg::Point p3 = autoware_utils::create_point(2.0, 0.0, 0.0);
  ...
  {
    geometry_msgs::msg::Point p1 = autoware_utils::create_point(0.0, 0.0, 0.0);
    geometry_msgs::msg::Point p2 = autoware_utils::create_point(5.0, 5.0, 0.0);
    geometry_msgs::msg::Point p3 = autoware_utils::create_point(10.0, 0.0, 0.0);
...
{
  using autoware_utils::calc_interpolated_point;
  using autoware_utils::create_point;
  using autoware_utils::create_quaternion;
  using autoware_utils::create_quaternion_from_rpy;
  using autoware_utils::deg2rad;
  using autoware_utils::is_driving_forward;
...
{
  using autoware_utils::calc_interpolated_pose;
  using autoware_utils::create_point;
  using autoware_utils::create_quaternion;
  using autoware_utils::create_quaternion_from_rpy;
  using autoware_utils::deg2rad;
...
TEST(geometry, getTwist)
{
  ...
  // getTwist
  {
    auto t_out = autoware_utils::create_twist(velocity, angular);
...
TEST(geometry, getTwistNorm)
{
  ...
  twist_with_covariance.twist = geometry_msgs::build<geometry_msgs::msg::Twist>()
                                  ...
                                  .angular(create_vector3(0.1, 0.2, 0.3));
  EXPECT_NEAR(autoware_utils::calc_norm(twist_with_covariance.twist.linear), 5.0, epsilon);
}
...
TEST(geometry, is_twist_covariance_valid)
{
  ...
  EXPECT_EQ(autoware_utils::is_twist_covariance_valid(twist_with_covariance), false);

  twist_with_covariance.covariance.at(0) = 1.0;
  EXPECT_EQ(autoware_utils::is_twist_covariance_valid(twist_with_covariance), true);
}

TEST(geometry, intersect)
{
  using autoware_utils::create_point;
  using autoware_utils::intersect;
                              ...
                              // sharing) compared to SAT and boost::geometry::intersect
{
  {  // 2 triangles with intersection
    autoware_utils::Polygon2d poly1;
    autoware_utils::Polygon2d poly2;
    ...
    EXPECT_TRUE(autoware_utils::intersects_convex(poly1, poly2));
    EXPECT_TRUE(autoware_utils::sat::intersects(poly1, poly2));
  }
  {  // 2 triangles with no intersection (but they share an edge)
    ...
    boost::geometry::correct(poly1);
    boost::geometry::correct(poly2);
    EXPECT_FALSE(autoware_utils::intersects_convex(poly1, poly2));
    EXPECT_FALSE(autoware_utils::sat::intersects(poly1, poly2));
  }
...
TEST(geometry, intersectPolygonRand)
{
  ...
  autoware_utils::StopWatch<std::chrono::nanoseconds, std::chrono::nanoseconds> sw;
  ...
  for (auto vertices = 3UL; vertices < max_vertices; ++vertices) {
    ...
    for (auto i = 0; i < polygons_nb; ++i) {
      polygons.push_back(autoware_utils::random_convex_polygon(vertices, max_values));
    }
    ...
    for (auto i = 0UL; i < polygons.size(); ++i) {
      for (auto j = 0UL; j < polygons.size(); ++j) {
        ...
        sw.tic();
        const auto gjk = autoware_utils::intersects_convex(polygons[i], polygons[j]);
        if (gjk) {
        ...
        sw.tic();
        const auto sat = autoware_utils::sat::intersects(polygons[i], polygons[j]);
        if (sat) {
...
TEST(geometry, PolygonTriangulation)
{
  ...
  using autoware_utils::triangulate;
...
{
  std::vector<autoware_utils::Polygon2d> polygons;
  std::vector<std::vector<autoware_utils::Polygon2d>> triangulations;
  constexpr auto polygons_nb = 100;
  constexpr auto max_vertices = 10;
  constexpr auto max_values = 1000;
  ...
  for (auto vertices = 4UL; vertices < max_vertices; ++vertices) {
    ...
    for (auto i = 0; i < polygons_nb; ++i) {
      auto polygon_opt = autoware_utils::random_concave_polygon(vertices, max_values);
    ...
    for (auto i = 0UL; i < polygons.size(); ++i) {
      for (auto j = 0UL; j < polygons.size(); ++j) {
        ...
        sw.tic();
        bool gjk_intersect = autoware_utils::test_intersection(
          triangulations[i], triangulations[j], autoware_utils::intersects_convex);
        if (!gjk_intersect) {
          gjk_no_intersect_ns += sw.toc();
        } else {
          gjk_intersect_ns += sw.toc();
        ...
        sw.tic();
        bool sat_intersect = autoware_utils::test_intersection(
          triangulations[i], triangulations[j], autoware_utils::sat::intersects);
        if (!sat_intersect) {
          sat_no_intersect_ns += sw.toc();
        } else {
          sat_intersect_ns += sw.toc();
